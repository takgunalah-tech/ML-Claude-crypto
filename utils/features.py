...
def _clip_norm(s: pd.Series, lo: float = -1.0, hi: float = 1.0) -> pd.Series:
    """Clip and scale series to [lo, hi]."""
    rng = s.max() - s.min()
    if rng == 0:
...
def compute_macro_risk_state(macro_dfs: dict[str, pd.DataFrame]) -> pd.Series:
    ...
    Steps:
      ...
      2. Align on common hourly index (ffill ≤ 4 to bridge weekends)
    ...
    aligned = [b.reindex(common_idx).ffill(limit=4) for b in feature_blocks]
    ...
    # Reindex to full common_idx (fill any gaps)
...
def compute_pivot_points(df: pd.DataFrame) -> pd.DataFrame:
    ...
    pivots = daily[['P', 'R1', 'R2', 'S1', 'S2']].reindex(df_idx.index, method='ffill')
...
def compute_anchor_features(df: pd.DataFrame, prefix: str) -> pd.DataFrame:
    ...
    adx    = adx_df.iloc[:, 0].rename('adx')   # ADX column
    ...
    bb_mid = bb.iloc[:, 2]  # middle band
    bb_width = _safe_div(bb.iloc[:, 0] - bb.iloc[:, 1], bb_mid)  # upper - lower / mid
    ...
    # Breakout prob: closer to high + volume surge → higher prob
...
def compute_altcoin_features(
    ...
    out['MACD_sig']  = macd_df.iloc[:, 1]
    out['MACD_hist'] = macd_df.iloc[:, 2]

    bb = _get_ta().bbands(close, length=20)
    out['BB_upper']  = bb.iloc[:, 0]
    out['BB_lower']  = bb.iloc[:, 1]
    out['BB_mid']    = bb.iloc[:, 2]
    out['BB_width']  = _safe_div(bb.iloc[:, 0] - bb.iloc[:, 1], bb.iloc[:, 2])
    out['BB_pct']    = bb.iloc[:, -1]  # %b
